...
/**
 * Ensure GitLab API URL ends with /api/v4
 */
...
async function promptForGitLabConfig(options) {
  ...
  if (options.gitlabUrl) {
    return { gitlabUrl: normalizeGitlabApiUrl(options.gitlabUrl) };
  }
  ...
  return { gitlabUrl: normalizeGitlabApiUrl(gitlabUrl) };
}
...
`;
  ...
  const workflowFiles = ['merge-request-review.yml', 'issue-triage.yml', 'manual-invoke.yml', 'build-mr-context.js'];
...
async function update(options = {}) {
  ...
  newGitlabUrl = normalizeGitlabApiUrl(newGitlabUrl);
  if (options.gitlabUrl) {
